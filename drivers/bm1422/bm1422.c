				  ...
				  uint8_t register_address, uint8_t count)
{
	...
	uint8_t cntl1_before;
        ...
        // BM1422 needs to be in standby for write access
	ret = bm1422_get_reg(dev, &cntl1_before, BM1422_CNTL1, 1);
	if (ret) {
		return ret;
	}

        ret = bm1422_reg_access(bm1422_data,
				  BM1422_WRITE_REG,
				  BM1422_CNTL1, &zero, 1);
        if (ret) {
		return ret;
	...
	ret = bm1422_reg_access(bm1422_data,
        ...
        if (register_address == BM1422_CNTL1) {
                return ret;
        }
...
{
        return bm1422_get_reg(dev, status, BM1422_STATUS_REG, 1);
}
...
static int bm1422_software_reset(const struct device *dev)
{
	...
	return bm1422_reg_write_mask(dev, BM1422_CNTL2, 
					BM1422_CNTL2_SOFT_RST_MSK, 
					BM1422_CNTL2_SOFT_RST_MODE(1));
}
				...
				const struct sensor_value *val)
{
	...
	switch (val->val1) {
	case 0:
		odr = BM1422_OSA_0HZ781;
		break;
	case 1:
		odr = BM1422_OSA_1HZ563;
		break;
	case 3:
		odr = BM1422_OSA_3HZ125;
		break;
	case 6:
		odr = BM1422_OSA_6HZ25;
		break;
	case 12:
		odr = BM1422_OSA_12HZ5;
		break;
        case 25:
		odr = BM1422_OSA_25HZ;
		break;
	case 50:
		...
		break;
	case 100:
		odr = BM1422_OSA_100HZ;
		break;
	case 200:
		odr = BM1422_OSA_200HZ;
		break;
        case 400:
		odr = BM1422_OSA_400HZ;
		break;
	case 800:
		odr = BM1422_OSA_800HZ;
		break;
	case 1600:
		odr = BM1422_OSA_1600HZ;
		break;
	case 3200:
		odr = BM1422_OSA_3200HZ;
		break;
	case 6400:
		odr = BM1422_OSA_6400HZ;
		break;
	default:
		return -EINVAL;
	}

	return bm1422_reg_write_mask(dev, BM1422_ODCNTL,
				      BM1422_ODCNTL_OUT_ODR_MSK,
				      BM1422_ODCNTL_OUT_ODR_MODE(odr));
}
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
static int bm1422_set_gsel(const struct device *dev,
				const struct sensor_value *val)
{
	...
	case 8:
		...
		break;
	case 16:
		gsel = BM1422_GSEL_16G;
		break;
	case 32:
		gsel = BM1422_GSEL_32G;
		break;
	case 64:
			    ...
			    const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_FULL_SCALE:
                return bm1422_set_gsel(dev, val);
	default:
		return -ENOTSUP;
	}
